...
export function SocketProvider({ children }) {
    ...
    const [socketInsta, setsocketInsta] = useState(io("http://192.168.1.182:4040"));

    // 在組件創建時初始化 socket 並在組件卸載時斷開連接
    useEffect(() => {
        const newSocket = io('http://192.168.1.182:4040');
        setsocketInsta(newSocket);
    ...
    return (
        <SocketContext.Provider value={socketInsta}>
            {children}
